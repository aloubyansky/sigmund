...
| P1.11 | Backend agreement interop test: the same claim through BC, `sq` and `gpg` yields the same outcome and reason. Covers key expiry judged at claim time (BC's check is new in P1.4, the other two apply their own rules), a missing key, a signature the key did not make, and a hybrid `.asc` where only some backends support the algorithm. Skips backends that are not installed rather than failing | §1.1, §3.4, §3.5 | P1.6 | todo |
| P1.12 | API-surface audit of the three backends once integrations settle: every public type, constructor and member in core justified by a caller outside the package, or narrowed. `BcRunner`'s constructors were public while taking a package-private `BcKeyStore`, so the surface is known to be wrong in places; `SqRunner` and `GpgRunner` are constructed directly by the CLI, so theirs cannot simply be closed. Factories stay the construction path | §2, §7 | P1.9 | todo |
