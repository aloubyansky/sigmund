...
- **Identity-validated signing** — configure expected OIDC identities and catch mismatches at signing time, before artifacts ship
...
| Identity validation at signing time | No | Yes (`allowedOidcIdentities`) |
...
|-----------|----------------------|----------------------|
| **OIDC identity validation** | None — accepts whatever identity the OIDC flow returns. A misconfigured CI workflow could sign with an unexpected identity. | Configurable via signer credentials — when an `oidc` credential is defined, `allowedOidcIdentities` rejects mismatched tokens after OIDC authentication completes but before the Fulcio certificate is requested and before any artifact is signed. In interactive mode, this means the user authenticates in the browser before learning their identity doesn't match. |
...
| (no equivalent) | `sigmund.yaml` → `signing.signer` + `signers.<name>.oidc` | Identity validation at signing time |
...
- **GitHub Actions only** — in sigstore-java 1.3.0, ambient OIDC is limited to GitHub Actions. Other CI systems require the interactive browser flow or a custom OIDC client configuration.

## Configuration with sigmund.yaml

For Sigstore-only signing, the minimal configuration is:
...
tools:
  sigstore:
    staging: false
    trusted-root: "/path/to/custom-root.json"  # optional, for air-gapped environments
    interactive: true                           # optional, enables browser OIDC flow
