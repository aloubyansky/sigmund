...
public final class CliTool {
    ...
    public static Result run(Map<String, String> env, String... command) {
        ...
        return execute(pb, null);
    }

    /**
     * Executes a command with additional environment variables and stdin input.
     * <p>
     * The {@code stdin} string is written to the process's standard input stream
     * and the stream is closed before reading output. This is used to pipe
     * passphrases to GPG via {@code --passphrase-fd 0}.
     *
     * @param env environment variables to add to the process, or null for none
     * @param stdin data to write to the process's standard input; must not be null
     * @param command the command and its arguments to execute
     * @return a {@link Result} containing exit code, stdout, and stderr
     * @throws IllegalArgumentException if command is null or empty
     * @throws UncheckedIOException if an I/O error occurs during execution
     * @throws RuntimeException if the process is interrupted or times out
     */
    public static Result runWithStdin(Map<String, String> env, String stdin, String... command) {
        validateCommand(command);
        ...
        return execute(pb, stdin);
    }

    private static Result execute(ProcessBuilder pb, String stdin) {
